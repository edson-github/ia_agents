...
DATABASE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "atendimentos.db")
...
def extrair_id_atendimento(question):
    # Primeiro tenta o padrão existente
    ...
    match = re.search(r'(?:^|\s)(\d+)(?:\s|$)', question)
...
def ask():
    ...
    print(f"Pergunta recebida: {question}")  # Log da pergunta
    atendimento_id = extrair_id_atendimento(question)
    print(f"ID extraído: {atendimento_id}")  # Log do ID extraído
